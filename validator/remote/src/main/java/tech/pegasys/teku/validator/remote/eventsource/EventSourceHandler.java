...
class EventSourceHandler implements BackgroundEventHandler {
  ...
  private void handleHeadV2Event(final String data) throws JsonProcessingException {
    ...
    final HeadV2Event.Data headEventData = headEvent.data();
    validatorTimingChannel.onHeadUpdate(
        headEventData.slot(),
        headEventData.currentEpochDependentRoot(),
        headEventData.nextEpochDependentRoot(),
        headEventData.block());
    if (generateEarlyAttestations) {
      validatorTimingChannel.onAttestationCreationDue(headEventData.slot());
    }
  }
